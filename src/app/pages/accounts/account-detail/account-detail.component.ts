...
export class AccountDetailComponent implements OnInit {
  ...
  monthlyActivityChartData: any[] = [];

  colorScheme = {
    domain: ["#00D68F", "#FF3D71", "#3366FF", "#FFAA00", "#00E096", "#A366FF"],
  };
  ...
  calculateAccountStats(transactions: Transaction[]): void {
    ...
    this.accountStats.depositAmount = deposits.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    this.accountStats.withdrawalAmount = withdrawals.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    this.accountStats.transferAmount = transfers.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    ...
    if (this.account) {
      ...
      this.accountStats.accountAge = Math.ceil(
        diffTime / (1000 * 60 * 60 * 24)
      );

      const months = this.accountStats.accountAge / 30;
      this.accountStats.monthlyAverage =
        months > 0 ? transactions.length / months : 0;
    }
  }
  ...
  prepareChartData(transactions: Transaction[]): void {
    ...
    this.transactionTypeChartData = [
      { name: "Dépôts", value: this.accountStats.totalDeposits },
      { name: "Retraits", value: this.accountStats.totalWithdrawals },
      { name: "Virements", value: this.accountStats.totalTransfers },
    ].filter((item) => item.value > 0);

    // Graphique 2: Activité mensuelle (6 derniers mois)
    ...
    this.monthlyActivityChartData = [
      {
        name: "Transactions",
        series: Object.entries(monthlyData).map(([name, value]) => ({
          name,
          value,
        })),
      },
      ...
      .open(ConfirmDialogComponent, {
        context: {
          ...
          message: `Êtes-vous sûr de vouloir supprimer ce compte ? <br><br>
            <strong>Numéro: </strong> ${this.formatAccountNumber(
              this.account.accountNumber
            )}<br>
            <strong>Solde:</strong> ${this.formatCurrency(
